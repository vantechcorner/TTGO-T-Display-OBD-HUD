#!/usr/bin/env python3
"""
Offline check: same style of ELM327 line normalization / mode-01 parsing as src/main.cpp (ASCII only).
...
Capture lines once from USB serial (or Bluetooth log), then replay here.
Note: PID 01 0D often returns 0 km/h when the vehicle is not moving.
"""
...
def elm_normalize_line(line: str) -> Optional[str]:
    """Compact ELM / ECU lines -> same normalization as firmware processElmLine."""
...
def main() -> int:
    ...
    def handle_line(raw: str) -> None:
        ...
        if n:
            print(f"OK  raw={raw!r} -> norm={n!r} -> parsed={hud}")
        else:
